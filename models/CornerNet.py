...
class pool(nn.Module):
    def __init__(self, dim, pool1, pool2):
        ...
        self.pool1 = pool1()  # top or bottom  pooling
        self.pool2 = pool2()  # left or right  pooling
...
class CornerNet(kp):
    def __init__(self, db=None):
        ...
        super(CornerNet, self).__init__(
            n, n_stack, dims, modules, out_dim,
            make_tl_layer=make_tl_layer,
            make_br_layer=make_br_layer,
            make_pool_layer=make_pool_layer,
            make_hg_layer=make_hg_layer,
...
aeloss = AELoss(pull_weight=1e-1, push_weight=1e-1, regr_weight=0, focal_loss=_neg_loss)
