...
from utils.image import crop_image, normalize_, color_jittering_, lighting_
...
def kp_detection(db, k_ind, data_aug, debug):
    ...
    tl_tags = np.zeros((batch_size, max_tag_len), dtype=np.int64)    # 每个值表示的是当前点的位置，y*width + x
    br_tags = np.zeros((batch_size, max_tag_len), dtype=np.int64)
    tag_masks = np.zeros((batch_size, max_tag_len), dtype=np.uint8)  # 每幅图像最多128个目标，每个目标有一个数值表示的tag
    tag_lens = np.zeros((batch_size,), dtype=np.int32)  # 每幅图片含有的目标框数量
